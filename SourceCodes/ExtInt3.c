...
int main(int argc, char** argv)
{
   // SYSTEMConfig(FCY, SYS_CFG_WAIT_STATES | SYS_CFG_PCACHE);
    ...
    TRISASET= _TRISA_TRISA14_MASK; // Set RA15 as input
  ...
  // TRISBSET= _TRISB_TRISB0_MASK|_TRISB_TRISB1_MASK; //RB0 and RB1 as switches
    ...
    // Interrupt Config
    INTCONSET=_INTCON_MVEC_MASK | _INTCON_INT3EP_MASK; //Enable Multi vectored interrupt and External Interrupt3 Edge Polarity Control bit- 1 = Rising edge 0 = Falling edge
    IFS0CLR= _IFS0_INT3IF_MASK; // Clear INT3 Flag bit
    IEC0SET=_IEC0_INT3IE_MASK; // INT3 enable
    IPC3bits.INT3IP= 0x04;// Priority
    IPC3bits.INT3IS= 0x03;//Sub priority
...
//ISR
void __ISR(_EXTERNAL_3_VECTOR, IPL7SOFT) EI3_ISR(void)
{
    // clear the interrupt flag
    IFS0CLR= _IFS0_INT3IF_MASK; // Clear INT4 Flag bit
    LED1_INV();
    delay(100);
}
//
