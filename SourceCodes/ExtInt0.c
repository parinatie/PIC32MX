...
int main(int argc, char** argv)
{
   // SYSTEMConfig(FCY, SYS_CFG_WAIT_STATES | SYS_CFG_PCACHE);
    ...
    //LEDs as output
    TRISDCLR= _TRISD_TRISD1_MASK|_TRISD_TRISD2_MASK;
    ...
    //TRISASET= _TRISA_TRISA15_MASK; // Set RA15 as input
   // TRISESET= _TRISE_TRISE9_MASK; // Set RE8 as input
    TRISDSET= _TRISD_TRISD0_MASK; // RD13 as input
  ...
  // TRISBSET= _TRISB_TRISB0_MASK|_TRISB_TRISB1_MASK; //RB0 and RB1 as switches
    ...
    // Interrupt Config
    INTCONSET=_INTCON_MVEC_MASK | _INTCON_INT0EP_MASK; //Enable Multi vectored interrupt and External Interrupt4 Edge Polarity Control bit- 1 = Rising edge 0 = Falling edge
    IFS0CLR= _IFS0_INT0IF_MASK; // Clear INT4 Flag bit
    IEC0SET=_IEC0_INT0IE_MASK; // INT4 enable
    IPC0bits.INT0IP= 0x04;// Priority
    IPC0bits.INT0IS= 0x03;//Sub priority
...
//ISR
void __ISR(_EXTERNAL_0_VECTOR, IPL7SOFT) EI0_ISR(void)
{
