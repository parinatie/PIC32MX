...
int main(int argc, char** argv)
{
    ...
    // Init ADC
    TRISDCLR= _TRISD_TRISD2_MASK; // LED2 
    TRISBSET= _TRISB_TRISB2_MASK; // ADC at RB2
    AD1PCFG&=~(1<<POT_RB3); //Analog input pin in Analog mode
    
        
    AD1CHSCLR= _AD1CHS_CH0NA_MASK; //
    AD1CHSbits.CH0SA=POT_RB3;// AN3
    
    AD1CHSCLR= _AD1CHS_CH0NB_MASK; //
    AD1CHSbits.CH0SB=POT_RB3;// AN3
    ...
    AD1CON2CLR=_AD1CON2_SMPI_MASK | _AD1CON2_BUFM_MASK |_AD1CON2_ALTS_MASK; //Interrupts at the completion of conversion for each sample/convert sequence
    //Buffer configured as one 16-word buffer ADC1BUF(15...0.), Always use MUX A input multiplexer settings
...
//     __builtin_enable_interrupts();
    ...
    {  
        ...
        //AD1CON1CLR=_AD1CON1_DONE_MASK; // Clear done bit           
        //adc_inprogress=1;
        //while(~IFS1&(1<<1));
        while(~AD1CON1&(1<<0));
...
////ISR
//void __ISR(_ADC_VECTOR, IPL7SOFT) ADC_ISR(void)
//{
//    
//    adc_inprogress=0;   
//    IFS1CLR=_IFS1_AD1IF_MASK; //Clear ADC INT flag
