...
#define T3_PSCLR1_1     (T3CONbits.TCKPS=0)
#define T3_PSCLR1_2     (T3CONbits.TCKPS=1)
#define T3_PSCLR1_4     (T3CONbits.TCKPS=2)
#define T3_PSCLR1_8     (T3CONbits.TCKPS=3)
#define T3_PSCLR1_16    (T3CONbits.TCKPS=4)
#define T3_PSCLR1_32    (T3CONbits.TCKPS=5)
#define T3_PSCLR1_64    (T3CONbits.TCKPS=6)
#define T3_PSCLR1_256   (T3CONbits.TCKPS=7)
...
int main(int argc, char** argv)
{
  ...
  // TRISBSET= _TRISB_TRISB0_MASK|_TRISB_TRISB1_MASK; //RB0 and RB1 as switches
    ...
    // Timer3 Config
     T3CONCLR=_T3CON_TCS_MASK; // Internal clock
     T3CONCLR=_T3CON_TGATE_MASK; //Gated time accumulation disabled
     //TRISCSET=_TRISC_TRISC1_MASK;//RC1 as gated input-T1CK
     T3_PSCLR1_256; // 1:256
     //T2_32BITMODE; //16 bit mode

     //TMR2=0;  
     TMR3=0; // TMR3 is higher 16-bit register
     //PR2=0xFFFF;    
     PR3=0xffff; 
    ...
    IFS0CLR= _IFS0_T3IF_MASK; // Clear T3 INT Flag bit
    IEC0SET=_IEC0_T3IE_MASK; // Timer3 Interrupt Enable
    IPC3bits.T3IP= 0x05;// Priority
    IPC3bits.T3IS= 0x03;//Sub priority
    __builtin_enable_interrupts();
    
    T3CONSET=_T3CON_ON_MASK; //Timer 3 on
...
//ISR
void __ISR(_TIMER_3_VECTOR, IPL7SOFT) T3_ISR(void)
{
   ...
   // count++;
    ...
    // clear the interrupt flag
     IFS0CLR= _IFS0_T3IF_MASK; // Clear T1 INT Flag bit // Clear T1 INT Flag bit
}
//
