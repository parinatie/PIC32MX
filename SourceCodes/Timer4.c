...
#define T4_PSCLR1_1     (T4CONbits.TCKPS=0)
#define T4_PSCLR1_2     (T4CONbits.TCKPS=1)
#define T4_PSCLR1_4     (T4CONbits.TCKPS=2)
#define T4_PSCLR1_8     (T4CONbits.TCKPS=3)
#define T4_PSCLR1_16    (T4CONbits.TCKPS=4)
#define T4_PSCLR1_32    (T4CONbits.TCKPS=5)
#define T4_PSCLR1_64    (T4CONbits.TCKPS=6)
#define T4_PSCLR1_256   (T4CONbits.TCKPS=7)
...
int main(int argc, char** argv)
{
  ...
  // TRISBSET= _TRISB_TRISB0_MASK|_TRISB_TRISB1_MASK; //RB0 and RB1 as switches
    ...
    // Timer4 Config
     T4CONCLR=_T4CON_TCS_MASK; // Internal clock
     T4CONCLR=_T4CON_TGATE_MASK; //Gated time accumulation disabled
     //TRISCSET=_TRISC_TRISC1_MASK;//RC1 as gated input-T1CK
     T4_PSCLR1_256; // 1:256
     //T2_32BITMODE; //16 bit mode

     //TMR2=0;  
     TMR4=0; // TMR3 is higher 16-bit register
     //PR2=0xFFFF;    
     PR4=0xffff; 
    ...
    IFS0CLR= _IFS0_T4IF_MASK; // Clear T3 INT Flag bit
    IEC0SET=_IEC0_T4IE_MASK; // Timer3 Interrupt Enable
    IPC4bits.T4IP= 0x05;// Priority
    IPC4bits.T4IS= 0x03;//Sub priority
    __builtin_enable_interrupts();
    
    T4CONSET=_T4CON_ON_MASK; //Timer 3 on
...
//ISR
void __ISR(_TIMER_4_VECTOR, IPL7SOFT) T4_ISR(void)
{
    ...
    // clear the interrupt flag
     IFS0CLR= _IFS0_T4IF_MASK; // Clear T4 INT Flag bit 
}
//
