...
int main(int argc, char** argv)
{
  ...
  // TRISBSET= _TRISB_TRISB0_MASK|_TRISB_TRISB1_MASK; //RB0 and RB1 as switches
    ...
    CNCONSET= _CNCON_ON_MASK; // Enable CN Module
    CNENSET=_CNEN_CNEN2_MASK|_CNEN_CNEN3_MASK; //Port pin input change notice enabled
    CNPUESET=_CNPUE_CNPUE2_MASK|_CNPUE_CNPUE3_MASK;//port pin pull-up enabled   
    PORTB;//Read corresponding PORT registers to clear mismatch condition on CN input pins
    IFS1CLR= _IFS1_CNIF_MASK; // Clear CN INT Flag bit
    IEC1SET=_IEC1_CNIE_MASK; // Change Notice Interrupt Enable
    IPC6bits.CNIP= 0x04;// Priority
    IPC6bits.CNIS= 0x03;//Sub priority
...
//ISR
void __ISR(_CHANGE_NOTICE_VECTOR, IPL7SOFT) CN_ISR(void)
{
